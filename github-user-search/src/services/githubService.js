...
// Fetch GitHub API key from the environment variables (use the VITE_ prefix for Vite projects)
...
export const fetchUserData = async (username) => {
  try {
    ...
    if (error.response && error.response.status === 404) {
      throw new Error('Not Found');  // Specific error for user not found
    }
    throw new Error('An error occurred. Please try again later.');  // Generic error message
  }
};
